      ...
      <ul className="m-0 flex list-none flex-col gap-3 p-0">
        {favoriteRates.map((fav) => (
          <li
            ...
            <div className="flex min-h-13 items-center gap-3 px-4 py-3 min-[640px]:gap-5">
              <button
                type="button"
                onClick={() => setPair(fav.from, fav.to)}
                aria-label={`Convert ${fav.from} to ${fav.to}`}
                className="flex min-w-0 flex-1 cursor-pointer items-center gap-3 text-left focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary min-[640px]:gap-5">
              ...
              <button
                type="button"
                onClick={() => removeFavorite({ from: fav.from, to: fav.to })}
                aria-pressed="true"
